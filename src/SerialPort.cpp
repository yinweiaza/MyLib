...
DWORD CSerialPort::Read(void* lpBuf, DWORD dwCount)
{
	...
	bResult = ReadFile(m_hComm, lpBuf, min(comstat.cbInQue, dwCount),  &cbReaded, /*NULL*/&overLapped);
	DWORD ret = WaitForSingleObject(overLapped.hEvent, 20);
	if(ret == WAIT_TIMEOUT)
	{
		CancelIo();
	}
	...
	{
		::GetOverlappedResult(m_hComm, &overLapped, &cbReaded, TRUE);
	}
...
DWORD CSerialPort::Write(const void* lpBuf, DWORD dwCount)
{
	...
	//	******DEBUG_ANNOTATION_MARKUP******
	//  	LPBYTE buf = (LPBYTE)lpBuf;
	//  	CString strMessage = _T("Will Write: \n");
	//  	CString strTemp;
	//  	for(int ii=0; ii<dwCount; ii++)
	//  	{
	//  		strTemp.Format(_T("%.2X "), buf[ii]);
	//  		strMessage += strTemp;
	//  		if(ii%32==7)
	//  			strMessage += _T(" ");
	//  		if(ii%32==15)
	...
	//  
	//  	CStdioFile stdFile;
	//  	CString strFilePath = _T("c:\\write.txt");
	//  	if(stdFile.Open(strFilePath,  
	//  		CFile::modeCreate | CFile::modeWrite|
	//  		CFile::typeText | CFile::modeNoTruncate))
	//  	{
	//  		stdFile.SeekToEnd();
	//  		stdFile.WriteString(strMessage);
	...
	ULONG cbSend = 0;
	BOOL bResult = TRUE;
	bResult = WriteFile(m_hComm, lpBuf,  dwCount, &cbSend, /*NULL*/&overLapped);

	DWORD ret = WaitForSingleObject(overLapped.hEvent, 100);
	if(ret == WAIT_TIMEOUT)
	{
		CancelIo();
	}
	...
	{
		::GetOverlappedResult(m_hComm, &overLapped, &cbSend, TRUE);
	}
...
// 
// 	if (!ReadFile(m_hComm, lpBuf, dwBytesRead, &dwBytesReaded, NULL))
// 	{
...
// //	//////******DEBUG_ANNOTATION_MARKUP******
// //  	LPBYTE buf = (LPBYTE)lpBuf;
// //  	CString strMessage = _T("Readed: \n");
// //  	CString strTemp;
// //  	for(int ii=0; ii<dwBytesReaded; ii++)
// //  	{
// //  		strTemp.Format(_T("%.2X "), buf[ii]);
// //  		strMessage += strTemp;
// //  		if(ii%32==7)
...
// //  
// //  	CStdioFile stdFile;
// //  	CString strFilePath = _T("c:\\read.txt");
// //  	if(stdFile.Open(strFilePath,  
// //  		CFile::modeCreate | CFile::modeWrite|
// //  		CFile::typeText | CFile::modeNoTruncate))
// //  	{
