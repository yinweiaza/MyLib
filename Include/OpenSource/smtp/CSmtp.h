...
#include <assert.h>
	...
	#include "../../../Include/OpenSource/openssl/ssl.h"
...
inline BOOL SendEMail(const CHAR *strSMTP, unsigned short usSmtpPort, const CHAR *SenderEMail, const CHAR * SendPsd, const CHAR * SenderUser, \
					  int ReceiverCount, const CHAR ReceiverUser[3][80], const CHAR ReceiverEMail[3][80], const CHAR *strTitle, const CHAR *strcontent)
{
	try
	{
		CSmtp mail;
		...
		mail.SetSMTPServer(strSMTP,usSmtpPort);
		mail.SetSecurityType(USE_SSL);

		mail.SetLogin(SenderEMail);//登陆信息
		mail.SetPassword(SendPsd);
		mail.SetSenderName(SenderUser);//邮件中的“发件人”名字
		mail.SetSenderMail(SenderEMail);//邮件中的“发件人”E-mail, 与登陆帐号要一致
		mail.SetReplyTo(SenderEMail);//自动回复地址
		mail.SetSubject(strTitle);//标题

		for( int i=0; i<ReceiverCount&&i<3; i++ )
			mail.AddRecipient(ReceiverEMail[i], ReceiverUser[i]);

		mail.SetXPriority(XPRIORITY_NORMAL);
		mail.SetXMailer("The Bat! (v3.02) Professional");
		mail.AddMsgLine(strcontent);
		// 		mail.ModMsgLine(5,"regards");//修改指定行
		// 		mail.DelMsgLine(2);//删除指定行
		//		mail.AddAttachment("c:/mc12test.txt");//添加附件失败， 需测试
		mail.Send();
	}
	catch(...)
	{
...
inline BOOL SendSmtpOneEMail(const CHAR *strSMTP, unsigned short usSmtpPort, const CHAR *SenderEMail, const CHAR * SendPsd, const CHAR * SenderUser, \
					  CHAR *ReceiverUser, const CHAR *strTitle, const CHAR *strcontent)
{
	try
	{
		CSmtp mail;
		...
		mail.AddRecipient(ReceiverUser, "");
