...
#define MEDIAINFOLIST_ASSIGN(_Name,_Name2) \
    ...
    typedef void* (__stdcall *MEDIAINFO_New)(const MediaInfo_Char*);
...
#define MEDIAINFO_TEST_STRING_STATIC \
    ...
    {
    public :
        MediaInfo(const String &LanPath)        {if (!MediaInfo_Module) MediaInfoDLL_Load(); if (!MediaInfo_Module) {Handle = NULL; return;}; Handle = MediaInfo_New(LanPath.c_str());};
