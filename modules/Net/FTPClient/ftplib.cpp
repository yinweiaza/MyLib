...
int ftplib::Connect(const char *host)
{
	...
	while(host[index++] != '\0')
		Len++;
	lhost = new char[Len+1];
	ZeroMemory(lhost,Len+1);
	memcpy(lhost,host,Len);
	lhost[Len]='\0';
	//lhost = strdup(host);
	//free(lhost);
	pnum = strchr(lhost,':');
	...
	int TimeOut=20000; //设置发送超时6秒
	if(::setsockopt(sControl,SOL_SOCKET,SO_SNDTIMEO,(char *)&TimeOut,sizeof(TimeOut))==SOCKET_ERROR)
	{
		perror("setsockopt");
		net_close(sControl);
		return 0;
	...
	TimeOut=20000;//设置接收超时6秒
	if(::setsockopt(sControl,SOL_SOCKET,SO_RCVTIMEO,(char *)&TimeOut,sizeof(TimeOut))==SOCKET_ERROR)
	{
		perror("setsockopt");
		net_close(sControl);
		return 0;
...
int ftplib::FtpOpenPasv(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
	...
	unsigned int n[6];
	sscanf(cp,"%d,%d,%d,%d,%d,%d",&n[2],&n[3],&n[4],&n[5],&n[0],&n[1]);
	for (int i=0; i<6; i++) v[i] = (unsigned char)n[i];
	...
	{
		char buf[256];
		sprintf(buf,"REST %I64d",mp_ftphandle->offset);
...
int ftplib::FtpRead(void *buf, int max, ftphandle *nData)
{
	...
	else
	{
		...
		FD_SET(nData->handle, &fdread);	//分配套接字句柄到相应的fd_set
		tv.tv_sec=2;					//这里我们打算让select等待两秒后返回，避免被锁死，也避免马上返回
		tv.tv_usec=0;
		select(0,&fdread,NULL,NULL,&tv);
		if(FD_ISSET(nData->handle,&fdread))
		{
 ...
 * return 1 if successful, 0 otherwise
 */
int ftplib::Size(const char *path, __int64 *size, transfermode mode)
{
   char cmd[256];
   int resp,rv=1;
   __int64 sz = 0;
   ...
   {
		if (sscanf(mp_ftphandle->response, "%d %I64d", &resp, &sz) == 2) *size = sz;
		else rv = 0;
   }
   return rv;
}
...
int ftplib::Quit()
{
	...
	if (FtpSendCmd("QUIT",'2',mp_ftphandle) == 1)
	{
		shutdown(mp_ftphandle->handle,2);
...
ftphandle* ftplib::RawOpenEx(const char *path, accesstype type, transfermode mode, off64_t offset)
{
	mp_ftphandle->offset = offset;
	return RawOpen(path, type, mode);
}
...
int ftplib::ReadCmdLine(char *buf,int max)
{
//
//return readline(buf, max, mp_ftphandle);
	//
	fd_set rfd;
	fd_set wfd;
	rfd.fd_count = 2;
	...
	FD_SET(mp_ftphandle->handle, &wfd);
//	tv = ctl->idletime;
	int rv = 0; // = select(mp_ftphandle->handle+1, fd, fd, NULL, &tv);
	int nTries= 3;
	do 
	{
		FD_ZERO(&rfd);
		FD_SET(mp_ftphandle->handle, &rfd);
		rv = select(mp_ftphandle->handle+1, &rfd,  NULL, NULL, &tv);
	} while (rv == 0 && nTries--);
	if (FD_ISSET(mp_ftphandle->handle,&rfd) )//测试sock是否可读，即是否网络上有数据
	{
		//recvfrom(sock,buffer,256,.....);//接受网络数据
		TRACE(_T("can Read\n"));
		rv = net_read(max, buf, max);
	}// end if break; 
/*	if (FD_ISSET(mp_ftphandle->handle, &wfd))
	{
		TRACE(_T("can write\n"));
	}*/
/*	if (rv == -1)
	...
	//
//	int nRead = net_read(ctl->handle,ctl->cput,ctl->cleft);		

	return rv;//readline(buf, max, mp_ftphandle);
}
char *ftplib::GetResp()
{
