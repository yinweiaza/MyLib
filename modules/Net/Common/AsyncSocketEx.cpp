...
public:
	...
	BOOL RemoveSocket(CAsyncSocketEx *pSocket, int &nSocketIndex)
	{
		...
		MSG msg;
		while (PeekMessage(&msg, m_hWnd, WM_MESSAGE_NET_ASYNCSOCK_NOTIFY + nSocketIndex, WM_MESSAGE_NET_ASYNCSOCK_NOTIFY + nSocketIndex, PM_REMOVE));
	...
	void RemoveLayers(CAsyncSocketEx *pOrigSocket)
	{
		...
		MSG msg;
		while (PeekMessage(&msg, m_hWnd, WM_MESSAGE_NET_ASYNCSOCK_SEND, WM_MESSAGE_NET_ASYNCSOCK_SEND, PM_REMOVE))
		{
	...
	{
		if (message>=WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	)
		{
			//Verify parameters
			ASSERT(hWnd);
			...
			if (message<static_cast<UINT>(WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	+pWnd->m_nWindowDataSize)) //Index is within socket storage
			{
				//Lookup socket and verify if it's valid
				CAsyncSocketEx *pSocket=pWnd->m_pAsyncSocketExWindowData[message-WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	].m_pSocket;
...
#ifndef NOLAYERS
		else if (message == WM_MESSAGE_NET_ASYNCSOCK_SEND) //Notification event sent by a layer
		{
			//Verify parameters, lookup socket and notification message
			//Verify parameters
			ASSERT(hWnd);
...
#endif //NOLAYERS
		else if (message == WM_MESSAGE_NET_ASYNCSOCK_REPLY)
		{
		...
		else if (message == WM_MESSAGE_NET_ASYNCSOCK_VERIFY)
		{
			//Verify parameters, lookup socket and notification message
			//Verify parameters
			if (!hWnd)
		...
		else if (message == WM_TIMER)
		{
			...
			PostMessage(hWnd, socket->m_SocketData.nSocketIndex + WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	, socket->m_SocketData.hSocket, FD_CLOSE);
			return 0;
		}
...
BOOL CAsyncSocketEx::Create(UINT nSocketPort /*=0*/, int nSocketType /*=SOCK_STREAM*/, long lEvent /*=FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/, LPCTSTR lpszSocketAddress /*=NULL*/, int nFamily /*=AF_INET*/)
{
...
#endif //NOLAYERS
	{
		...
		else
		{
			...
			{
				m_lEvent = lEvent;
				if (WSAAsyncSelect(m_SocketData.hSocket, GetHelperWindowHandle(), m_SocketData.nSocketIndex+WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	, FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE) )
				{
					Close();
					return FALSE;
				}
...
BOOL CAsyncSocketEx::Connect(LPCTSTR lpszHostAddress, UINT nHostPort)
{
	...
	if (m_SocketData.nFamily == AF_INET)
	{
		...
		if (sockAddr.sin_addr.s_addr == INADDR_NONE)
		{
			...
			m_hAsyncGetHostByNameHandle=WSAAsyncGetHostByName(GetHelperWindowHandle(), WM_MESSAGE_NET_ASYNCSOCK_REPLY, lpszAscii, m_pAsyncGetHostByNameBuffer, MAXGETHOSTSTRUCT);
			if (!m_hAsyncGetHostByNameHandle)
				return FALSE;
...
BOOL CAsyncSocketEx::Attach(SOCKET hSocket, long lEvent /*= FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/)
{
	...
	{
		m_lEvent = lEvent;
		return !WSAAsyncSelect(m_SocketData.hSocket, GetHelperWindowHandle(), m_SocketData.nSocketIndex+WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	, FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE);
	}
	else
#endif //NOLAYERS
	{
...
BOOL CAsyncSocketEx::AsyncSelect( long lEvent /*= FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/ )
{
...
#endif //NOLAYERS
	{
		...
		if ( !WSAAsyncSelect(m_SocketData.hSocket, GetHelperWindowHandle(), m_SocketData.nSocketIndex+WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	, lEvent) )
			return TRUE;
		else
			return FALSE;
	}
...
BOOL CAsyncSocketEx::TriggerEvent(long lEvent)
{
	...
	if (m_pFirstLayer)
	{
		...
		pMsg->pLayer=0;
		BOOL res=PostMessage(GetHelperWindowHandle(), WM_MESSAGE_NET_ASYNCSOCK_SEND, (WPARAM)m_SocketData.nSocketIndex, (LPARAM)pMsg);
		if (!res)
			delete pMsg;
		return res;
	}
...
#endif //NOLAYERS
		return PostMessage(GetHelperWindowHandle(), m_SocketData.nSocketIndex+WM_MESSAGE_NET_ASYNCSOCK_NOTIFY	, m_SocketData.hSocket, lEvent%0xFFFF);
...
void CAsyncSocketEx::AddCallbackNotification(const t_callbackMsg& msg)
{
	...
	if(m_pendingCallbacks.size() == 1 && m_SocketData.nSocketIndex != -1)
		PostMessage(GetHelperWindowHandle(), WM_MESSAGE_NET_ASYNCSOCK_VERIFY, (WPARAM)m_SocketData.nSocketIndex, 0);
}
