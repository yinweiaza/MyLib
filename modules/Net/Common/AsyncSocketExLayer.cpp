...
BOOL CAsyncSocketExLayer::TriggerEvent(long lEvent, int nErrorCode, BOOL bPassThrough /*=FALSE*/ )
{
	...
	BOOL res=PostMessage(m_pOwnerSocket->GetHelperWindowHandle(), WM_MESSAGE_NET_ASYNCSOCK_SEND, (WPARAM)m_pOwnerSocket->m_SocketData.nSocketIndex, (LPARAM)pMsg);
	if (!res)
		delete pMsg;
	return res;
}
	...
	else if (m_nFamily == AF_INET6 || m_nFamily == AF_UNSPEC)
	{
		...
		for (res1 = res0; res1; res1 = res1->ai_next)
		{
			...
			if (m_nFamily == AF_UNSPEC)
			{
				...
				{
					if (WSAAsyncSelect(m_pOwnerSocket->m_SocketData.hSocket, m_pOwnerSocket->GetHelperWindowHandle(), m_pOwnerSocket->m_SocketData.nSocketIndex+WM_MESSAGE_NET_ASYNCSOCK_NOTIFY, FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE) )
					{
						m_pOwnerSocket->Close();
						res = FALSE;
						continue;
				...
				if (m_pOwnerSocket->m_pendingCallbacks.size())
					PostMessage(m_pOwnerSocket->GetHelperWindowHandle(), WM_MESSAGE_NET_ASYNCSOCK_VERIFY, (WPARAM)m_pOwnerSocket->m_SocketData.nSocketIndex, 0);
			}
